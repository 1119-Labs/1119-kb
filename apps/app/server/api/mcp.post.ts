...
/**
 * POST /api/mcp
 * MCP (Model Context Protocol) endpoint for coding agents (Cursor, Claude Code, etc.).
 * Authenticate with user API key: Authorization: Bearer <key> or x-api-key: <key>.
 *
 * Tools:
 * - listSources: list all knowledge sources with versions (sorted by time)
 * - searchKnowledge: run a query against the knowledge base, return structured answer
 */
export default defineEventHandler(async (event) => {
  if (event.method !== 'POST') {
    setResponseStatus(event, 405)
    setHeader(event, 'Content-Type', 'application/json')
    return { error: 'method_not_allowed', message: 'Method Not Allowed', statusCode: 405 }
  }
